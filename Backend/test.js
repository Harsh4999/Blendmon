...
const test=async (url2)=>{//Insat post
    const res = await axios.get(url2+'?__a=1', {
        headers: {
          accept:
     ...
     let arr=[];
         ...
         if(res.data.graphql.shortcode_media.is_video){
            ...
            console.log(beta)
            console.log(res.data.graphql.shortcode_media.owner.username);
            arr.push('vid');
            arr.push(beta);
         }
         else{
...
test('https://www.instagram.com/p/CYuCPxLBOl9/');